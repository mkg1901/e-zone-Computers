...
export function SaleForm({customers,banks,legacy,newStock,onSave}:{customers:Customer[];banks:Bank[];legacy:Stock[];newStock:NewStockProduct[];onSave:(fd:FormData)=>Promise<void>}){
 ...
 const matches=newStock.filter(p=>`${p.id} ${p.accessoryType} ${p.brand} ${p.modelNo} ${p.serials.filter(s=>!s.sold).map(s=>s.serialNumber).join(' ')}`.toLowerCase().includes(query.trim().toLowerCase()));
 ...
 return <form className={source==='new'?'newBilling':undefined} onSubmit={async e=>{e.preventDefault();if(pending.current)return;const fd=new FormData(e.currentTarget);if(source==='new'&&draft){setError('Add the selected item to the bill, or cancel it before saving.');return}if(source==='new'&&(!items.length||!fd.get('buyer'))){setError('Select a buyer and add at least one item.');return}if(source==='new'&&items.some(r=>newStock.find(p=>p.id===r.productId)?.serialRequired&&r.serials.length!==Number(r.quantity))){setError('Select one serial number for every unit of a serialized item.');return}setError('');if(source==='new')fd.set('items',JSON.stringify(items.map(row=>({new_product_id:row.productId,quantity:Number(row.quantity),unit_price:Number(row.price),serial_numbers:row.serials}))));pending.current=true;setSubmitting(true);try{await onSave(fd)}finally{pending.current=false;setSubmitting(false)}}}>
  <fieldset className="saleFields" disabled={submitting}>
   ...
   {source==='legacy'?<div className="grid2">
    ...
    {!draft&&<div className="catalogResults" aria-label="Matching items">{matches.map((p,i)=><button className={active===i?'active':''} type="button" key={p.id} onClick={()=>selectProduct(p)}><span><b>{p.accessoryType} · {p.brand} {p.modelNo}</b><small>{p.id} · {p.serialRequired?'Serial required':'Quantity stock'}</small></span><span>{p.quantity} available <strong>Select</strong></span></button>)}{!matches.length&&<div className="empty">No matching available items.</div>}</div>}
    {draft&&selectedProduct&&<div className="billItemEditor" aria-label="Selected product">
     <div className="saleLineHead"><div><h3>{selectedProduct.accessoryType} · {selectedProduct.brand} {selectedProduct.modelNo}</h3><small className="blockMuted">{selectedProduct.id}</small></div><button type="button" className="button secondary small" onClick={closeItem}>Cancel selection</button></div>
     <div className="selectedStockFacts"><span><b>{selectedProduct.quantity}</b> available</span><span>Purchase price / unit: <b>{inr(selectedProduct.purchasePrice)}</b></span></div>
